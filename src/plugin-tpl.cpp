...
const char BINARY_NAME[]    = "file-tpl";
const char LOAD_PROCEDURE[] = "file-tpl-load";
const char SAVE_PROCEDURE[] = "file-tpl-save";
...
void query()
{
    ...
    gimp_install_procedure(LOAD_PROCEDURE, "Loads TPL Textures", "Loads TPL Textures", "veebs", "Copyright (C) 2025 veebs", "2025", "TPL Texture", NULL, GIMP_PLUGIN, G_N_ELEMENTS(load_arguments), G_N_ELEMENTS(load_return_values), load_arguments, load_return_values);
    gimp_install_procedure(SAVE_PROCEDURE, "Saves TPL Textures", "Saves TPL Textures", "veebs", "Copyright (C) 2025 veebs", "2025", "TPL Texture", "RGB*", GIMP_PLUGIN, G_N_ELEMENTS(export_arguments), 0, export_arguments, NULL);

    gimp_register_load_handler(LOAD_PROCEDURE, "tpl", "");
    gimp_register_save_handler(SAVE_PROCEDURE, "tpl", "");
}
...
gboolean load_image(const gchar *filename, gint32 *imageID, GError **error)
{
    ...
    do {
        ...
        if(img.Load(&stream)){
            ...
            *imageID = gimp_image_new(img.GetImage(0)->mWidth, img.GetImage(0)->mHeight, GIMP_RGB);

            auto layers = img.GetImages();

            uint32_t idx = 0;
            for(auto lyr : layers){
                GdkPixbuf* imgBuff = gdk_pixbuf_new_from_data(lyr->GetData(), GDK_COLORSPACE_RGB, TRUE, 8, lyr->mWidth, lyr->mHeight, lyr->mWidth*4, NULL, NULL);
                gint32 layerID = gimp_layer_new_from_pixbuf(*imageID, std::format("image_{}", idx++).data(), imgBuff, 100, GIMP_LAYER_MODE_NORMAL, 0.0, 0.0);
